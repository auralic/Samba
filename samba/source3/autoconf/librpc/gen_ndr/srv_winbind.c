...
static bool api_winbind_SendToSam(struct pipes_struct *p)
{
	const struct ndr_interface_call *call;
	struct ndr_pull *pull;
	struct ndr_push *push;
	...
	call = &ndr_table_winbind.calls[NDR_WINBIND_SENDTOSAM];

	r = talloc(talloc_tos(), struct winbind_SendToSam);
	if (r == NULL) {
		return false;
	}
	...
	if (DEBUGLEVEL >= 10) {
		NDR_PRINT_FUNCTION_DEBUG(winbind_SendToSam, NDR_IN, r);
	}

	r->out.result = _winbind_SendToSam(p, r);
	...
	if (DEBUGLEVEL >= 10) {
		NDR_PRINT_FUNCTION_DEBUG(winbind_SendToSam, NDR_OUT | NDR_SET_VALUES, r);
	}
...
static const struct api_struct api_winbind_cmds[] = 
{
	...
	{"WINBIND_SENDTOSAM", NDR_WINBIND_SENDTOSAM, api_winbind_SendToSam},
};
