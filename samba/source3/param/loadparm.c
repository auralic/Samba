...
*/
	...
	lpcfg_string_set(Globals.ctx, &Globals.ntp_signd_socket_directory,"/usr/local/samba/var/lib/ntp_signd");
    
	//s = talloc_asprintf(talloc_tos(), "%s/samba_kcc", get_dyn_SCRIPTSBINDIR());
	s = talloc_asprintf(talloc_tos(), "%s/samba_kcc", "/usr/local/samba/sbin");
	if (s == NULL) {
		smb_panic("init_globals: ENOMEM");
	}
	...
	s = talloc_asprintf(talloc_tos(), "%s/samba_dnsupdate", "/usr/local/samba/sbin");
	if (s == NULL) {
		smb_panic("init_globals: ENOMEM");
	}
	...
	s = talloc_asprintf(talloc_tos(), "%s/samba_spnupdate", "/usr/local/samba/sbin");
	if (s == NULL) {
		smb_panic("init_globals: ENOMEM");
	}
...
static struct loadparm_context *setup_lp_context(TALLOC_CTX *mem_ctx)
{
	...
	const struct loadparm_s3_helpers *s3_fns = loadparm_s3_helpers();
	lp_ctx->s3_fns = s3_fns;
	lp_ctx->globals = s3_fns->globals;
	lp_ctx->flags = s3_fns->flags;
	lp_ctx->loaded = true;
	//printf("*****%d\n",lp_ctx->loaded);
	if (lp_ctx == NULL) {
